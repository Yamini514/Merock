...
const TYPE_META = {
  Customer: { icon: MessageSquare, label: 'Client',    color: 'bg-blue-50 text-blue-600',      border: 'border-blue-200/60' },
  Property: { icon: Building2,     label: 'Property',  color: 'bg-indigo-50 text-indigo-600',  border: 'border-indigo-200/60' },
  Referral: { icon: TrendingDown,  label: 'Referral',  color: 'bg-emerald-50 text-emerald-600', border: 'border-emerald-200/60' },
  Match:    { icon: Clock,         label: 'Enquiry',    color: 'bg-amber-50 text-amber-600',    border: 'border-amber-200/60' },
}
const DEFAULT_META = { icon: Bell, label: 'Notification', color: 'bg-slate-100 text-slate-500', border: 'border-slate-200/60' }
...
export default function Alerts() {
  const { user } = useAuth()
  // Viewer may read + mark-read (allowlisted PUTs) but not delete alerts.
  const canDismiss = !isViewer(user)
  const [activeTab, setActiveTab] = useState('all')
  const [busyId, setBusyId] = useState(null)
  const [err, setErr] = useState('')

  const fetcher = useCallback(() => listAlerts({ page_size: 100 }), [])
  ...
  async function markRead(id) {
    ...
    setBusyId(id)
    setErr('')
    try { await markAlertRead(id); refetch() } catch (e) { setErr(e.message) } finally { setBusyId(null) }
  }
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in max-w-3xl">
      <PageHeader
        ...
        actions={
          unread > 0 && (
            <Button variant="secondary" size="sm" onClick={markAllRead} className="w-full sm:w-auto justify-center">
